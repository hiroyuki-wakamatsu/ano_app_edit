...
class MakeEstimatePdf:
    ...
    def _getYGridAnno(self, input_data):
        ...
        type = self.type
        if type == 1:
            fqitems_type = "anoteanoter_notes"
        elif type == 2:
    ...
    def __dataAnnoConfirm(self, input_data):
        ...
        # 本見積価格の注文番号有無を判定
        confirmAnnoFlg1 = 0
        for item in input_data["anoteanoter"]:
            # 注釈番号取得
        ...
        """
        confirmAnnoFlg = confirmAnnoFlg1
        # confirmAnnoFlg= 0 -> 注釈フィールドが空
        # confirmAnnoFlg= 1 -> 注釈フィールドに文字列が入っている
    ...
    # インスタンス関数
    # 社内確認情報データ有無判定
    # __説明：社内確認情報のレコードがあるか判定
    # __呼出元：クラス estimatepdf_main.ExeMakeEstimatePdf
    # __引数：input_data -> PDFに描画する全データが含まれるタプル型データ
    # __戻り値： confirmConFlg
    #
    def __dataConConfirm(self, input_data):
        # 返却フラグ初期化
        confirmConFlg = 0

        # 社内確認のデータ有無を判定
        if input_data["anoteanoter_confirm"]:
            confirmConFlg = 1
        else:
            confirmConFlg = 0

        # confirmConFlg= 0 -> 社内確認情報フィールドが空
        # confirmConFlg= 1 -> 社内確認情報フィールドに文字列が入っている
        return confirmConFlg
    ...
    # ★メモ：Fortinetの場合のみ本見積価格, 参考価格, 次年度参考価格を分ける
    # 　　　　その他製品は”製品情報”で固定
    #
    # インスタンス関数
    ...
    def _drawContentDataRows(self, item):
        ...
        if res > 1:  # 品名の文字数が"MAX_ZNW_SKU_JP=37"より大きかったら改行
            strlists = splitStr(item["znw_sku_jp"], self.MAX_ZNW_SKU_JP)
            for text in strlists:
                tmp_x = self.VERTICAL_X1 + (
                    self.PADDING_BOTTOM * 2
                )  # PADDING_BOTTOMは微調整部分。以下使用部分も同様
                tmp_y = (
                    self.index_y
                    - (self.GRID_Y * switchCount)
                    + self.PADDING_BOTTOM * 6.5
                )
    ...
    def _drawContentData(self, input_data):
        ...
        # 見積種別 1:本見積価格 2:参考価格 3:次年度参考価格--------------------
        if type == 1:
            fqitems_type = "anoteanoter"  # ★メモ：その他製品は本見積のみ。fqitems_typeのみ代入。
            sum_data = "quotation_price"  # 本見積合計金額
        elif type == 2:
            fqitems_type = "fqitems_ref"
    ...
    # ★メモ：Fortinet以外は本見積のみの為、type=1固定
    #
    # インスタンス関数
    ...
    #       type           -> 見積種別 1:本見積価格 2:参考価格 3:次年度参考価格。★Fortinet以外は1固定。
    ...
    def _drawContentAnnotationDataRows(self, item):
        ...
        self.pdf.line(
            self.LEFT_X,
            self.index_y - (self.PADDING_BOTTOM * 4),
            self.RIGHT_X,
            self.index_y - (self.PADDING_BOTTOM * 4),
        )
    ...
    # Todo 製品社内確認事項(その他製品のみ)body作成 開始-------------------------------------------------------------------------

    # インスタンス関数(他インスタンス関数から呼出し)
    # 社内確認情報body作成_1
    # __説明：社内確認事項body部分のヘッダーを作成(確認No,社内確認情報)
    # __呼出元：__drawContentconfirm
    # __引数：なし

    def _drawContentConfirmHead(self):
        font_size = 8
        self.pdf.setFont(self.font, font_size)
        self.pdf.setLineWidth(0.1)
        ...
        x = self.LEFT_X + (self.GRID_X / 2.2)
        y = (start_y - self.GRID_Y) + (self.GRID_Y / 2)
        self.pdf.drawCentredString(x, y, "確認")
        self.pdf.drawCentredString(x, y - (self.GRID_Y / 2.5), "No")
        ...
        x = self.VERTICAL_X2 + ((self.VERTICAL_X3 - self.VERTICAL_X2) / 2)
        y = (start_y - self.GRID_Y) + (self.GRID_Y / 3)
        self.pdf.drawCentredString(x, y, "社内確認情報")
    ...
    # __呼出元：__drawContentconfirm
    # __引数：item -> 注釈データ(タプル型)
    def _drawContentConfirmDataRows(self, item):
        deepCount = 1
        switchCount = 1
        # switchCount = 0.8
        ...
        self.pdf.drawCentredString(tmp_x, tmp_y, item["confirm_line_number"])

        # 改行コード処理---始---------------------------------------------------------
        anno_data = []
        tmp_str = item["confirm_data"]
    ...
    # __呼出元：__drawContentConfirm
    # __引数：input_data -> 注釈データ(辞書型)
    def _drawContentConfirmData(self, input_data):
        font_size = 8
        self.pdf.setFont(self.font, font_size)
        self.pdf.setLineWidth(0.1)
        ...
        type = self.type
        if type == 1:
            fqitems_type = "anoteanoter_confirm"
        elif type == 2:
            fqitems_type = ""
        elif type == 3:
            fqitems_type = ""
        ...
        # 注釈行の数だけ描画を繰り返す
        for item in input_data[fqitems_type]:
            self._drawContentConfirmDataRows(item)

    # インスタンス関数
    # 社内確認情報body作成_4
    # __説明：社内確認情報行の枠と実データを描写する関数
    # __呼出元：クラス estimatepdf_main.ExeMakeEstimatePdf
    # __引数：
    #       input_data -> 社内確認情報データ(辞書型)
    #       type       -> 1->本見積価格 2->参考価格 3->次年度参考価格
    def __drawContentConfirm(self, input_data, type: int()):
        # 描画座標設定
        x_coordinate = self.LEFT_X
        y_coordinate = self.index_y - self.GRID_Y
        start_y = y_coordinate
        ...
        # 社内確認情報ヘッダ描画
        self._drawContentConfirmHead()

        # インデックスの更新(1Grid分Y軸方向に下げる)
        self.index_y = self.index_y - self.GRID_Y
        # 注釈実データ描画
        self._drawContentConfirmData(input_data)

        # 参考
        # self._drawContentAnnotationData(input_data)

    # Todo 製品社内確認事項(その他製品のみ)body作成 終了-------------------------------------------------------------------------
    ...
    def __drawdraft(
        ...
        self.pdf.setStrokeColor(colors.red)
        self.pdf.rect(
            self.GRID_X * grid_x, self.GRID_Y * grid_y, grid_x_span, grid_y_span, fill=0
        )
        self.pdf.setFillColor(colors.black)
        self.pdf.setStrokeColor(colors.black)
    ...
    def __makeDictData(self, estimateid: int(), hosyu):
        ...
        self.hosyu = hosyu  # 保守用フラグ。Fort以外はFalse

        # その他製品　ヘッダーオブジェクトモデル取得
        estimateobj = AnotherQuotationHeader.objects.get(id=estimateid)
        # その他製品　製品オブジェクトモデル取得
        mainset = estimateobj.anotherquotationmain_set.all().order_by("number")
        # その他製品　注釈オブジェクトモデル取得
        notesset = estimateobj.anotherquotationnotes_set.all().order_by("number")
        # その他製品　社内確認オブジェクトモデル取得
        confirmset = estimateobj.anotherquotationconfirm_set.all().order_by("number")
        ...
        # Todo STEP1_製品詳細 実データ 配列作成--------------------------------------
        # 1_製品情報データ部分に記載する辞書型データを配列として作成
        # PDFに表示する項目：No,品名,型番,数量,標準価格,仕切率,ご提供単価,ご提供価格,見積合計,注釈番号
        main_dict = {
            "line_number": "",  # No.
            "znw_sku_jp": "",  # 品名
            "znw_sku": "",  # 型番
            ...
            "list_price": "",  # 標準価格
            "discount_rate": "",  # 仕切率
            "offer_unit_price": "",  # ご提供単価
            "offer_price": "",  # ご提供価格
            "offer_price_sum": "",  # 見積合計
            "notes_number": "",  # 注釈番号
        }
        # 最終的にmainitems[]に製品情報のレコード行分の辞書型データの配列が作成される
        mainitems = []
        # mainset:製品情報のオブジェクト
        if mainset:
            for obj in mainset:
                main_dict = {
                    "line_number": str(obj.number),  # No.
                    "znw_sku_jp": str(obj.znw_sku_jp),  # 品名
                    "znw_sku": str(obj.znw_sku),  # 型番
                    "qty": str(obj.qty),  # 数量
                    "list_price": "¥{:,}".format(obj.lp),  # 標準価格
                    # "discount_rate": "{:,%}".format(obj.discount_rate),   仕切率(Forti)
                    "discount_rate": str(obj.discount_rate) + "%",  # 仕切率(その他製品)
                    "offer_unit_price": "¥{:,}".format(obj.offer_unit_price),  # ご提供単価
                    "offer_price": "¥{:,}".format(obj.offer_price),  # ご提供価格
                    "offer_price_sum": "¥{:,}".format(obj.offer_price),  # 見積合計
                    "annotation_no": str(obj.notes_number),  # 注釈番号
                }
                # リストに追加する
                main_dict_list = (
                    main_dict.copy()
                )  # 辞書型"main_dict"の値を辞書型"main_dict_list"に格納
                mainitems.append(
                    main_dict_list
                )  # 辞書型"main_dict_list"の値を辞書型配列"mainitems"に項番の数分、追加していく

        # Todo STEP2_注釈データ 配列作成--------------------------------------
        # 1_注釈情報部分に記載する辞書型データを配列として作成
        # PDFに表示する項目：注釈No.,注釈本文
        notes_dict = {"number": "", "notes": ""}

        # 最終的にnotesitems[]に注釈情報のレコード行分の辞書型データの配列が作成される
        notesitems = []
        # notesset:注釈情報のオブジェクト
        if notesset:
            for obj in notesset:
                notes_dict = {
                    "anno_line_number": str(obj.number),  # 注釈No.
                    "anno_data": str(obj.notes),  # 注釈本文
                }
                # リストに追加する
                notes_dict_list = notes_dict.copy()
                notesitems.append(notes_dict_list)

        # Todo STEP3_社内確認情報データ 配列作成--------------------------------------
        # 1_社内確認情報部分に記載する辞書型データを配列として作成
        # PDFに表示する項目：確認No.,社内確認情報本文
        notes_dict = {"number": "", "internal_confirm": ""}

        # 最終的にnotesitems[]に注釈情報のレコード行分の辞書型データの配列が作成される
        confirmsitems = []
        # confirmset:社内確認情報のオブジェクト
        if confirmset:
            for obj in confirmset:
                notes_dict = {
                    "confirm_line_number": str(obj.number),  # 社内確認No.
                    "confirm_data": str(obj.internal_confirm),  # 社内確認情報本文
                }
                # リストに追加する
                notes_dict_list = notes_dict.copy()
                confirmsitems.append(notes_dict_list)

        # Todo STEP_END_クラス引数作成(辞書型)-------------------------------------------------------------------------
        # STEP1～3の配列を利用して辞書型データを作成
        another_data = {
            ...
            "anoteanoter": mainitems,  # 製品情報データ(配列)
            "anoteanoter_notes": notesitems,  # 注釈情報データ(配列)
            "anoteanoter_confirm": confirmsitems,  # 社内確認情報データ(配列)
            # "offer_price_sum": "¥{:,}".format(estimateobj.main_offer_price_sum),  # 合計金額
            "footer_items": footer_sentence,  # フッター文言(配列)
            "stamp_1_id": "/static/anoapp/media/伊藤.png",  # 職印_左端
            "stamp_2_id": "/static/anoapp/media/鈴木.png",  # 職印_中央
            "stamp_3_id": "/static/anoapp/media/吉田.png",  # 職印_右端
        }
        return another_data
